...
##############################################################################
# OptiVolt - Fix Dashboard avec Bonnes Métriques cAdvisor
# 
# Problème: Les requêtes utilisent name="optivolt-docker" mais cAdvisor
# utilise image="python:3.11-slim" ou id contient le nom
##############################################################################
...
echo ""
echo "🔧 Correction des métriques dashboard..."
echo ""
...
# Dashboard avec requêtes correctes
    ...
    "panels": [
      ...
      },
      {
        ...
        "targets": [
          {
            "refId": "A",
            "expr": "rate(container_cpu_usage_seconds_total{image=\"python:3.11-slim\"}[1m]) * 100",
            "legendFormat": "Standard (python:3.11-slim)"
          },
          {
            "refId": "B",
            "expr": "rate(container_cpu_usage_seconds_total{image=\"python:3.11-alpine\"}[1m]) * 100",
            "legendFormat": "Alpine (python:3.11-alpine)"
          },
          {
            "refId": "C",
            "expr": "rate(container_cpu_usage_seconds_total{image=\"alpine:3.18\"}[1m]) * 100",
            "legendFormat": "Minimal (alpine:3.18)"
          }
        ],
        "fieldConfig": {
          "defaults": {
          ...
          "overrides": [
            ...
            {
              "matcher": {"id": "byRegexp", "options": "Alpine.*"},
              "properties": [{"id": "color", "value": {"fixedColor": "#5794F2", "mode": "fixed"}}]
            },
            {
              "matcher": {"id": "byRegexp", "options": "Minimal.*"},
      ...
      },
      {
        ...
        "targets": [
          {
            "refId": "A",
            "expr": "container_memory_usage_bytes{image=\"python:3.11-slim\"} / 1024 / 1024",
            "legendFormat": "Standard (python:3.11-slim)"
          },
          {
            "refId": "B",
            "expr": "container_memory_usage_bytes{image=\"python:3.11-alpine\"} / 1024 / 1024",
            "legendFormat": "Alpine (python:3.11-alpine)"
          },
          {
            "refId": "C",
            "expr": "container_memory_usage_bytes{image=\"alpine:3.18\"} / 1024 / 1024",
            "legendFormat": "Minimal (alpine:3.18)"
          }
        ],
        "fieldConfig": {
...
if echo "$RESPONSE" | grep -q '"status":"success"'; then
    echo "✅ Dashboard corrigé avec métriques cAdvisor !"
    echo ""
    echo "🔧 Changements:"
    echo "  • Requêtes CPU: image=\"python:3.11-slim|alpine\""
    echo "  • Requêtes RAM: image=\"python:3.11-slim|alpine\""
    echo "  • Compatible avec format cAdvisor réel"
    echo ""
